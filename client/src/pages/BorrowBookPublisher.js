...
function BorrowBookPublisher() {
    const [access,setAccess]=useState("");
    useEffect(() => {
        authenticate();
    },[]);
    async function authenticate() {
        const response=await fetch('http://localhost:9000/publisher/canread',{
            method: 'POST',
            credentials: 'include',
            headers: {
    ...
    async function getBooks(searchTerm="",searchParams="isbn") {
     const response=await fetch(`http://localhost:9000/publisher/books`,{
         method: 'POST',
         credentials: 'include',
         headers: {
             'content-Type': 'application/json'
    ...
    async function reserveBook(isbn){
        ...
        const data=await response.json();
        if(data.status==='already reserved') {
            toast.warning('Already Reserved',{position: toast.POSITION.TOP_CENTER});
...
export default BorrowBookPublisher;